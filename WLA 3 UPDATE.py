...
# ==========================================
# 5. TAMPILAN DASHBOARD (5 TAB STRATEGIS)
# ==========================================
...
with tab3:
    ...
    # Inisialisasi Session State agar hasil simulasi diteruskan ke Page 4 & Page 5
...
# ------------------------------------------
# PAGE 4: ANALISIS KEBUTUHAN STAFF (PASCA SIMULASI)
# ------------------------------------------
with tab4:
...
# ------------------------------------------
# PAGE 5: REKOMENDASI AKHIR DSS + KALKULATOR LEMBUR
# ------------------------------------------
with tab5:
    ...
    # Hitung kebutuhan staff berdasarkan data simulasi
    ...
    if sisa_over == 0 and sisa_under == 0:
        ...
        * **Tindakan Jangka Panjang (Solusi Permanen):** Rekrut **{selisih_staff} orang operator baru** untuk menyeimbangkan kapasitas lini secara berkelanjutan.
        * **Tindakan Jangka Pendek (Alternatif Lembur):** Jika penambahan *headcount* belum dapat dilakukan dalam waktu dekat, berlakukan **skema jam kerja lembur (*overtime*)** untuk operator yang masih *Overload* dengan rincian di bawah ini:
        """)
        
        # ==========================================
        # ⏱️ FITUR KALKULATOR JAM LEMBUR
        # ==========================================
        st.markdown("### ⏱️ Rincian Estimasi Kebutuhan Jam Lembur (*Overtime*) per Shift")
        
        # Filter hanya operator yang Overload
        df_lembur = df_final[df_final["Kategori_WLA"] == "Overload"].copy()
        
        # Hitung kelebihan waktu dalam menit & jam
        df_lembur["Kelebihan_Menit"] = df_lembur["Total_Waktu_Kerja_Menit"] - jam_kerja_efektif
        df_lembur["Kebutuhan_Lembur_Jam"] = df_lembur["Kelebihan_Menit"] / 60.0
        
        # Tampilkan Tabel Lembur
        df_tampil_lembur = df_lembur[["Operator", "Percent_WLA", "Total_Waktu_Kerja_Menit", "Kelebihan_Menit", "Kebutuhan_Lembur_Jam"]]
        
        st.dataframe(df_tampil_lembur.style.format({
            "Percent_WLA": "{:.1f}%",
            "Total_Waktu_Kerja_Menit": "{:.1f} Menit",
            "Kelebihan_Menit": "{:.1f} Menit",
            "Kebutuhan_Lembur_Jam": "{:.2f} Jam"
        }), use_container_width=True)
        
        total_jam_lembur_lini = df_lembur["Kebutuhan_Lembur_Jam"].sum()
        st.warning(f"💡 **Total Jam Lembur Lini:** Diperlukan total **{total_jam_lembur_lini:.2f} jam lembur/shift** yang terbagi ke operator *Overload* di atas untuk menyelesaikan target produksi harian.")
